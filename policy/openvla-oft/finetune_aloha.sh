...
TORCHRUN_BIN=${TORCHRUN_BIN:-/root/autodl-tmp/conda_envs/RoboTwin5090/bin/torchrun}
$TORCHRUN_BIN --standalone --nnodes 1 --nproc-per-node 2 vla-scripts/finetune.py \
  --vla_path /root/autodl-tmp/checkpoints/openvla/openvla-7b \
  --data_root_dir /root/tensorflow_datasets \
  --dataset_name aloha_stack_three_blocks \
  --run_root_dir /root/autodl-tmp/checkpoints/openvla-oft \
  ...
  --lora_rank 32 \
  --wandb_entity "local" \
  --wandb_project "openvla-oft-robotwin" \
  --run_id_note stack_three_blocks
